...
//    public static let auth = Auth0.authentication(clientId: (Bundle.main.object(forInfoDictionaryKey: "mjyRrvseENhu2d0LLm0LxfNyRIXb67qw") as? String ?? ""), domain: (Bundle.main.object(forInfoDictionaryKey: "echoflow-dev.uk.auth0.com") as? String ?? ""))
//       private static let scopes = "openid profile offline_access preferred_username"
//       private static let domain = Bundle.main.object(forInfoDictionaryKey: "echoflow-dev.uk.auth0.com") as? String ?? ""
//
    
    let credentialsManager = CredentialsManager(authentication: Auth0.authentication())

    
    public func beginOAuthFlow() async throws {
...
//            request.httpBody = "grant_type=authorization_code&code=\(code)&redirect_uri=echo://\(redirectScheme)".data(using: .utf8)
...
//        }
    ...
    public func validToken() async throws -> Credentials {
...
//            let authUrl = URL(string: authUrlBase)?.appending(queryItems: [.init(name: "client_id", value: clientKey), .init(name: "response_type", value: "code"), .init(name: "state", value: "echo"), .init(name: "redirect_uri", value: "echo://\(redirectScheme)"), .init(name: "duration", value: "permanent"), .init(name: "scope", value: "creddits,modnote,modcontributors,modmail,modconfig,subscribe,structuredstyles,vote,wikiedit,mysubreddits,submit,modlog,modposts,modflair,save,modothers,read,privatemessages,report,identity,livemanage,account,modtraffic,wikiread,edit,modwiki,modself,history,flair")])
//            
//            if let authUrl {
//                let authSession = ASWebAuthenticationSession(url: authUrl, callbackURLScheme: redirectScheme, completionHandler: { callbackUrl, error in
//                    if error == nil {
...
//    }
//    
//    public func handleOauth(response: URL) {
//        OAuthSwift.handle(url: response)
//    }
//    
//    
//    public func logout() async {
//        await keychainManager.delete(key: tokenKey)
